...
subGrid s (r, c) =
  [ s (r', c') | r' <- (\x -> concat $ filter (elem x) blocks) r, c' <- (\x -> concat $ filter (elem x) blocks) c
  ]
...
openPositions sud =
  [ (row, col) | row <- positions, col <- positions, sud (row, col) == openPosition
  ]
...
solveSudoku sud
  | consistent solution = solution
  | otherwise = error "Unsolvable sudoku!"
  where
    solution = head (solve sud)
...
subtree sud = [extend sud (row, col, v) | v <- freeAtPos sud (row, col)]
  where
    (row, col) = head (openPositions sud)
